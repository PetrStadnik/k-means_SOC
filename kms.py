...
def createGraph(xname, x, yname, y, title, savename, logver=False, showval=False):
    ...
    plt.savefig("graphs/test29L/"+savename+".eps", format="eps")
    if logver:
        plt.clf()
        plt.xlabel(xname)
        plt.ylabel(yname)
        ...
        plt.yscale("log")
        plt.plot(x, y)
        plt.savefig("graphs/test29L/" + savename + "_log.eps", format="eps")
    plt.clf()
...
def kms(fn, nclus, stop=1e-5, maxIteration=300, allowBGR=True, positionWeight=None):
    ...
    #c = initKMS(arr, nclus)
    c = np.load("arrays/MQ/centroids/bgr" + str(n) + "k" + str(k) + ".npy")[0]
    centroids_arr = []
...
if __name__ == "__main__":
    ...
    for n in range(3, 11):
        dbi_arr = []
        chi_arr = []
        f = open("testLog/test29L.txt", "a")
        f.write("\n -------------------------*****************-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-\n bgr: " + str(n))
        for k in range(2, 13):
            sttime = time.time()
            centroids_arr, DB_index, CH_index, img, sum_list, r_list, delta_list = kms(fn="imgs/BGR/bgr"+ str(n) +".jpg", maxIteration=300, nclus=k, allowBGR=True,
                                                              positionWeight=None)
            np.save("arrays/LL/centroids/bgr" + str(n) + "k" + str(k) + ".npy", centroids_arr)
            np.save("arrays/LL/delta/bgr" + str(n) + "k" + str(k) + ".npy", delta_list)
            np.save("arrays/LL/price/bgr" + str(n) + "k" + str(k) + ".npy", sum_list)
            np.save("arrays/LL/r_function/bgr" + str(n) + "k" + str(k) + ".npy", r_list)
            ...
            cv2.imwrite("savedImages/test29L/bgr"+str(n)+"_k" + str(k) + ".jpg", img)
            f.write("\n ------------------------------------------\n k: " + str(k))
            f.write("\n DB: " + str(DB_index))
        ...
        createGraph(xname="počet shluků", x=range(2, len(dbi_arr)+2), yname="DBI", y=dbi_arr, title="Davis-Bouldin Index", savename="DBIbgr"+str(n),
                    showval=True)
        createGraph(xname="počet shluků", x=range(2, len(chi_arr)+2), yname="DBI", y=chi_arr, title="Calinski-Harabasz Index", savename="CHIbgr"+str(n),
                    showval=True)
        np.save("arrays/LL/DBI/bgr" + str(n) + ".npy", dbi_arr)
        np.save("arrays/LL/CHI/bgr" + str(n) + ".npy", chi_arr)
        f.close()
